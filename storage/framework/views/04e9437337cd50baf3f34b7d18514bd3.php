        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                <div class="search-container">
                    <form method="GET" action="<?php echo e(route($routeName)); ?>" class="search-input-group">
                        <input type="text" name="search" value="<?php echo e(request('search')); ?>"
                               placeholder="<?php echo e($searchPlaceholder); ?>" class="search-input">

                        <!-- Preserve existing filters -->
                        <?php $__currentLoopData = $filters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $filter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php if(request()->has($filter['name']) && $filter['name'] !== 'search'): ?>
                    ...
                    <?php if(!empty($filters)): ?>
                        <div style="position: relative;">
                            <button id="filterButton-<?php echo e($id); ?>" class="filter-btn" onclick="toggleFilters('<?php echo e($id); ?>', event)">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
                                </svg>
                            </button>

                            <div id="filterDropdown-<?php echo e($id); ?>" class="filter-dropdown" style="display: none;">
                                <form method="GET" action="<?php echo e(route($routeName)); ?>">
                                    <input type="hidden" name="search" value="<?php echo e(request('search')); ?>">

                                    <?php $__currentLoopData = $filters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $filter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php if($filter['type'] === 'select'): ?>
                                            <label for="filter_<?php echo e($filter['name']); ?>"><?php echo e($filter['label']); ?></label>
                                            <select name="<?php echo e($filter['name']); ?>" id="filter_<?php echo e($filter['name']); ?>">
                                                <option value=""><?php echo e($filter['all_option'] ?? 'All'); ?></option>
                                        ...
                                        <?php elseif($filter['type'] === 'date'): ?>
                                            <label for="filter_<?php echo e($filter['name']); ?>"><?php echo e($filter['label']); ?></label>
                                            <input type="date" id="filter_<?php echo e($filter['name']); ?>" name="<?php echo e($filter['name']); ?>" value="<?php echo e(request($filter['name'])); ?>">
                                        <?php elseif($filter['type'] === 'checkbox'): ?>
                                            <label>
                                                <input type="checkbox" name="<?php echo e($filter['name']); ?>" value="<?php echo e($filter['value'] ?? 1); ?>" <?php echo e(request($filter['name']) ? 'checked' : ''); ?>>
                ...
                <!-- Results Summary -->
                <?php if(method_exists($data, 'count') && method_exists($data, 'total')): ?>
                    <div class="mb-4">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            Showing <?php echo e($data->count()); ?> of <?php echo e($data->total()); ?> results
                            <?php if($showArchived && request('archived')): ?>
                                (archived)
                            <?php endif; ?>
                        </p>
                    </div>
                ...
                <div class="table-wrapper">
                    <table class="w-full bg-white dark:bg-gray-800" id="<?php echo e($id); ?>-table">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <?php $__currentLoopData = $columns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $column): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider"
                                    style="width: <?php echo e($column['width'] ?? 'auto'); ?>;">
                                    <?php if($sortable && isset($column['sortable']) && $column['sortable']): ?>
                                        <a href="<?php echo e(route($routeName, array_merge(request()->query(), ['sort' => $column['key'], 'direction' => request('sort') == $column['key'] && request('direction') == 'asc' ? 'desc' : 'asc']))); ?>"
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="table-row-hover <?php echo e(isset($item->archived) && $item->archived ? 'archived-row' : ''); ?>"
                                <?php if(isset($columns[0]['clickable']) && $columns[0]['clickable']): ?>
                                    onclick="handleRowClick(event, <?php echo e($item->id); ?>)"
                                <?php endif; ?>>
                                <?php $__currentLoopData = $columns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $column): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <td class="px-4 py-4 <?php echo e($column['class'] ?? 'text-sm text-gray-900 dark:text-gray-100'); ?>"
                                        <?php if(isset($column['no_click']) && $column['no_click']): ?>
                        ...
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <tr>
                                <td colspan="<?php echo e(count($columns)); ?>" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center space-y-2">
                                        <svg class="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="<?php echo e($emptyIcon); ?>"></path>
                                        </svg>
                                        <p class="text-lg font-medium"><?php echo e($emptyMessage); ?></p>
                                    </div>
                                </td>
                            </tr>
                        <?php endif; ?>
...
<?php /**PATH C:\Users\reece.downs\Downloads\fm-selection-j560679\public_html\laravel12\resources\views/components/data-table.blade.php ENDPATH**/ ?>