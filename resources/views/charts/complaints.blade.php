    ...
    <div class="charts-container">
        <div class="charts-wrapper">
            ...
            <div class="controls-section">
                <form method="GET" action="{{ route('charts.complaints') }}" id="chartForm">
                    <div class="controls-grid">
                            ...
                            <div class="searchable-dropdown">
                                <button type="button" class="dropdown-button" id="chart-dropdown-btn" aria-expanded="false">
                                    <span id="chart-selected-text">
                                        {{ $chartTypes[$chartType] ?? 'Select Chart Type' }}
                                    </span>
                                ...
                                <input type="hidden" name="chart" id="chart-type" value="{{ $chartType }}">
                            </div>
                        </div>
                        ...
                        <div class="control-group">
                            ...
                            <select name="range" id="time-range" class="control-select" onchange="toggleCustomDateFields()">
                                <option value="30-days" {{ $timeRange === '30-days' ? 'selected' : '' }}>Last 30 Days</option>
                                <option value="3-months" {{ $timeRange === '3-months' ? 'selected' : '' }}>Last 3 Months</option>
                                <option value="6-months" {{ $timeRange === '6-months' ? 'selected' : '' }}>Last 6 Months</option>
                                <option value="12-months" {{ $timeRange === '12-months' ? 'selected' : '' }}>Last 12 Months</option>
                            ...
                            <div class="searchable-dropdown">
                                <button type="button" class="dropdown-button" id="sites-dropdown-btn" aria-expanded="false">
                                    <span id="sites-selected-text">
                                        ...
                                        @elseif(count($siteIds) == 1)
                                            {{ $sites->where('id', $siteIds[0])->first()->name ?? 'Selected Site' }}
                                        @else
                                            {{ count($siteIds) }} sites selected
                                        @endif
                                    </span>
                                    <span class="dropdown-arrow">▼</span>
                                </button>
                                <div class="dropdown-menu" id="sites-dropdown-menu">
                                    ...
                                    <div class="dropdown-option" data-value="">
                                        <input type="checkbox" id="all-sites" {{ empty($siteIds) ? 'checked' : '' }}>
                                        <label for="all-sites"><strong>All Sites</strong></label>
                                    </div>
                                    @foreach($sites as $site)
                                        <div class="dropdown-option" data-value="{{ $site->id }}">
                                            <input type="checkbox" name="sites[]" value="{{ $site->id }}" id="site-{{ $site->id }}"
                                                {{ in_array($site->id, $siteIds ?? []) ? 'checked' : '' }}>
                    ...
                    <!-- Custom Date Range Fields -->
                    <div class="custom-date-fields {{ $timeRange === 'custom' ? 'show' : '' }}" id="custom-date-fields">
                        <div class="control-group">
            ...
            <div class="chart-section" id="chart-section">
                @if(isset($data))
                    <div class="chart-header">
                        ...
                        <p class="chart-subtitle">
                            @if($timeRange === 'custom' && $customFrom && $customTo)
                                {{ \Carbon\Carbon::parse($customFrom)->format('M j, Y') }} - {{ \Carbon\Carbon::parse($customTo)->format('M j, Y') }}
                            @else
                                {{ ucwords(str_replace(['-', '_'], ' ', $timeRange)) }}
                            @endif
                            @if(!empty($siteIds))
                                • Filtered by {{ count($siteIds) }} site(s)
                            @else
                                • All Sites
                            @endif
                        </p>
                    ...
                    <div class="{{ $data['type'] === 'doughnut' ? 'doughnut-container' : ($data['type'] === 'bar' ? 'bar-container' : 'chart-container') }}">
                    ...
                    <div class="chart-stats">
                        <div class="stat-item">
                            <p class="stat-value">{{ collect($data['values'])->sum() }}</p>
                            <p class="stat-label">Total</p>
                        </div>
                        <div class="stat-item">
                            <p class="stat-value">{{ collect($data['values'])->avg() ? number_format(collect($data['values'])->avg(), 1) : '0' }}</p>
                            <p class="stat-label">Average</p>
                        </div>
                        <div class="stat-item">
                            <p class="stat-value">{{ collect($data['values'])->max() ?? 0 }}</p>
                            <p class="stat-label">Peak</p>
                        </div>
                        <div class="stat-item">
                            <p class="stat-value">{{ collect($data['values'])->count() }}</p>
