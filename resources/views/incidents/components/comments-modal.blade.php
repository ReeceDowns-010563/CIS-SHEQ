...
<!-- Comments Modal -->
<div id="{{ $modalId }}" class="comments-modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); backdrop-filter: blur(3px);">
    <div class="comments-modal-content" style="position: relative; background-color: white; margin: 5% auto; padding: 0; border-radius: 12px; width: 90%; max-width: 600px; max-height: 80vh; box-shadow: 0 20px 40px rgba(0, 0, 0, 0.2); animation: modalSlideIn 0.3s ease;">
        ...
        <div class="comments-modal-body" style="padding: 24px; max-height: 50vh; overflow-y: auto;">
            ...
            <div id="{{ $modalId }}Content" style="display: none;">
                <div class="comments-list" id="{{ $modalId }}List" style="max-height: 300px; overflow-y: auto; margin-bottom: 20px;">
                    <!-- Comments will be loaded here -->
                </div>

                <form class="comment-form" id="{{ $modalId }}Form" style="border-top: 1px solid #e5e7eb; padding-top: 20px;">
                    @csrf
                    <textarea id="{{ $modalId }}Input" class="comment-input" placeholder="Add your comment..." required maxlength="1000" style="width: 100%; padding: 12px; border: 1px solid #d1d5db; border-radius: 8px; resize: vertical; min-height: 80px; font-family: inherit; font-size: 14px; transition: border-color 0.2s;"></textarea>
                    <button type="submit" class="comment-submit" id="{{ $modalId }}SubmitBtn" style="margin-top: 12px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; padding: 10px 20px; border-radius: 8px; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.2s;">
                        Post Comment
                    </button>
                </form>
            </div>
...
<script>
    // Comments modal functionality for {{ $modalId }}
    (function() {
