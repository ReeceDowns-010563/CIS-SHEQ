<x-app-layout>
            ...
            <div class="welcome-section">
                <div class="welcome-header">
                    ...
                    <div class="welcome-controls">
                        ...
                        {{-- Toggle only shows if user has access to both features --}}
                        @if($canAccessComplaints && $canAccessIncidents)
                        ...
                        @elseif($canAccessIncidents && !$canAccessComplaints)
                            {{-- Show only incidents label when user can only access incidents --}}
                            <div class="dashboard-toggle">
                                <button class="toggle-option active" data-view="incidents" disabled style="cursor: default;">
                                    Accident Reports
                                </button>
                            </div>
                        @elseif($canAccessComplaints && !$canAccessIncidents)
                            {{-- Show only complaints label when user can only access complaints --}}
                            <div class="dashboard-toggle">
                    ...
                    <div class="swapy-item stat-card" data-swapy-item="stat1">
                        ...
                        <div class="stat-content">
                            <h3 id="monthlyStatLabel">
                                @if($canAccessIncidents && !$canAccessComplaints)
                            ...
                            <p class="stat-description">{{ now()->format('F Y') }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="swapy-item stat-card" data-swapy-item="stat2">
                        <div class="stat-header">
                            ...
                            <div class="stat-trend" id="resolutionTrend">
                                ...
                                @endphp
                                {{ $avgTime == 0 ? 'No Data' : 'Average' }}
                            </div>
                        </div>
                        <div class="stat-content">
                            <h3>Resolution Time</h3>
                            <div class="stat-value" id="resolutionValue">{{ $avgTime }}</div>
                            <p class="stat-description" id="resolutionDesc">{{ $avgTime == 1 ? 'day' : 'days' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="swapy-slot table-full" data-swapy-slot="table1">
                    <div class="swapy-item table-section" data-swapy-item="table1">
                        <div class="table-header">
                            ...
                            <a href="@if($canAccessIncidents && !$canAccessComplaints){{ route('charts.incidents') ?? '#' }}@elseif($canAccessComplaints && !$canAccessIncidents){{ route('complaints.manage') ?? '#' }}@else{{ route('charts.incidents') ?? '#' }}@endif" class="table-action" id="tableViewAllLink">
                                View All
    ...
    <script>
        ...
        const dashboardData = {
            incidents: {
                monthly: {{ $monthlyIncidents ?? 0 }},
                avgResolution: {{ $averageClosureTimeIncidents ?? 0 }},
                open: {{ $openIncidents ?? 0 }},
                closed: {{ $closedIncidents ?? 0 }},
                trends: @json($incidentTrends ?? []),
                types: @json($incidentTypes ?? []),
                recent: @json($recentIncidents ?? [])
            },
            complaints: {
                monthly: {{ $monthlyComplaints ?? 0 }},
                avgResolution: {{ $averageClosureTime ?? 0 }},
                open: {{ $openComplaints ?? 0 }},
                closed: {{ $closedComplaints ?? 0 }},
                trends: @json($complaintTrends ?? []),
                types: @json($complaintTypes ?? []),
                recent: @json($recentComplaints ?? [])
            }
        };

        // User access permissions
        const userAccess = {
            canAccessIncidents: {{ $canAccessIncidents ? 'true' : 'false' }},
            canAccessComplaints: {{ $canAccessComplaints ? 'true' : 'false' }},
            hasToggle: {{ ($canAccessIncidents && $canAccessComplaints) ? 'true' : 'false' }}
        };

        // Available routes (fallback to # if route doesn't exist)
        const routes = {
            incidents: {
                manage: '{{ route("charts.incidents") ?? "#" }}',
                create: '#',
                edit: function(id) { return '{{ url("/incidents") }}/' + id + '/edit'; }
            },
            complaints: {
                manage: '{{ route("complaints.manage") ?? "#" }}',
                create: '{{ route("complaints.create") ?? "#" }}',
                edit: function(id) { return '{{ route("complaints.edit", ":id") ?? "#" }}'.replace(':id', id); }
            }
        };
        ...
        function initializeSwapySystem() {
            ...
            @if(isset($dashboardLayout) && $dashboardLayout)
                try {
            ...
            const saveLayout = debounce(function(data) {
                ...
                fetch('{{ route("dashboard.save-layout") }}', {
                    method: 'POST',
                    headers: {
        ...
        function setupResetButton(csrfToken) {
            document.getElementById('resetLayoutBtn').addEventListener('click', function() {
                if (confirm('Are you sure you want to reset the dashboard layout to default?')) {
                    ...
                    fetch('{{ route("dashboard.reset-layout") }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
