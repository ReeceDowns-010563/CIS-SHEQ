<x-app-layout>
    <x-slot name="header">
        ...
        <div class="header-container">
            ...
            <a href="{{ route('settings.users.create') }}" class="create-user-btn">
                Create User
            </a>
        </div>
    </x-slot>
    ...
    <div class="main-container">
        <div class="content-wrapper">
            ...
            <div class="filters-container">
                <form method="GET" action="{{ route('settings.users.index') }}" class="filters-form">
                    ...
                    <!-- Clear Filters -->
                    @if(request('search') || request('role') || request('show_archived'))
                        <div class="filter-group">
                            <a href="{{ route('settings.users.index') }}" class="clear-filters-btn">
                                Clear Filters
                            </a>
                        </div>
                    @endif
            ...
            @if(isset($users) && $users->total() > 0)
                <div class="results-summary">
                    ...
                    {{ request('show_archived') ? 'archived' : 'active' }} users
                    @if(request('search'))
                        for "{{ request('search') }}"
                    @endif
                    @if(request('role'))
            ...
            <div class="table-container">
                <table class="users-table">
                    ...
                    <tr>
                        <th>
                            <a href="{{ route('settings.users.index', array_merge(request()->query(), ['sort' => 'name', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}">
                                Name
                        ...
                        <th>
                            <a href="{{ route('settings.users.index', array_merge(request()->query(), ['sort' => 'email', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}">
                                Email
                        ...
                        <th>Status</th>
                        <th>
                            <a href="{{ route('settings.users.index', array_merge(request()->query(), ['sort' => 'created_at', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}">
                                Created
                    ...
                    @forelse($users ?? [] as $user)
                        <tr class="{{ $user->isArchived() ? 'archived-row' : '' }}">
                            <td>
                                <div>
                            ...
                            <td>
                                @if($user->role)
                                    ...
                                    @endphp
                                    <span class="role-badge role-color-{{ $colorIndex }}">
                            ...
                            <td>
                                <div class="action-links">
                                    @if(!$user->isArchived())
                                        <a href="{{ route('settings.users.edit', $user) }}" class="edit-link">
                                            Edit
                                        </a>
                                        @if($user->id !== auth()->id())
                                            <form method="POST" action="{{ route('settings.users.destroy', $user) }}" style="display: inline;" onsubmit="return confirm('Are you sure you want to archive this user? They will be hidden from active users but their data will be preserved.')">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    @else
                                        @if(auth()->user()->isAdmin())
                                            <a href="{{ route('settings.users.index', ['unarchive' => $user->id]) }}"
                                               class="unarchive-btn"
                                ...
                                @if(request('search') || request('role'))
                                    <div style="text-align: center;">
                                        ...
                                        <p style="color: #6b7280; margin-bottom: 1rem; font-size: 0.875rem;">
                                            No {{ request('show_archived') ? 'archived' : 'active' }} users match your current search criteria.
                                        </p>
                                        <a href="{{ route('settings.users.index') }}" class="clear-filters-btn" style="display: inline-block; width: auto; max-width: 200px;">
                                            Clear Filters
                                        </a>
                                    </div>
                                @else
                                    <div style="text-align: center;">
                                        ...
                                        <h3 style="font-size: 1rem; font-weight: 600; margin-bottom: 0.5rem;">
                                            No {{ request('show_archived') ? 'archived' : '' }} users found
                                        </h3>
                                        ...
                                        @if(!request('show_archived'))
                                            <a href="{{ route('settings.users.create') }}" class="create-user-btn" style="display: inline-block; width: auto; max-width: 200px;">
                                                Create User
                                            </a>
                                        @endif
                                    </div>
